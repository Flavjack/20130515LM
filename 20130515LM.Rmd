...
tab <- xl %>% 
  ...
  info_table(
    caption = "Potato genotypes (*Solanum tuberosum* L.) used for water deficit experiment with two commercial varieties and 13 genotypes from advanced breeding population developed by the International Potato Center (CIP)."
    , notes = "International Potato Center"
    , label = "Source:"
    )
  
# tab$table %>% sheet_write(gs, data = .,  sheet = "tab1")
...
tab <- fb %>%
  ...
  rename(Variable = `**Characteristic**`
         ...
         , `p-value` = `**p-value**`) %>% 
  info_table(caption = "Treatment comparison for 17 variables between Well-Watered (WW) and Water Deficit (WD) in 15 potato genotypes. The values for different variables are represented by the mean ± standard deviation with the significance under t-test with their respective p-values.")
...
```{r mpv, fig.cap= fig$caption, eval=T}
...
save_plot(plot = plot, "files/fig_1.tiff", base_height = 12, base_width = 12)
...
```{r cor, fig.cap=fig$caption, eval=T}
...
fig <- pdftools::pdf_convert(
  ...
  , filenames = "files/fig_2.tiff"
  , dpi = 600
  )

fig <- info_figure(
  caption = "Relationships among agro-morphological traits evaluated in well-watered (WW) and water deficit (WD) treatments based on Pearson correlation and Euclidean distance measured in 15 potato genotypes."
...
```{r mva, fig.cap= fig$caption, eval=T}
...
save_plot(plot = plot, "files/fig_3.tiff")
...
info <- tab$h2 %>% 
  ...
  info_table(
    caption = "Genetic variance (V.g), Error variance (V.e), Cullis (h2.c) and Piepho (h2.p) broad sense heritability in 15 potatoes genotypes. ")
...
save_plot(plot = plot, "files/fig_s1.tiff", base_width = 6.5, base_height = 5)
...
```{r pca, fig.cap=fig$caption, eval=T}
...
save_plot(plot = plot, "files/fig_s2.tiff", base_height = 10, base_width = 10)
...
fig <- png::readPNG("files/fig_potatos.png") %>%
  grid::rasterGrob(interpolate = TRUE)

save_plot("files/fig_s3.tiff", plot = fig, base_asp = 1)
```
