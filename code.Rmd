...
fb %>%
  ...
  mutate(`**Characteristic**` = recode(`**Characteristic**`,
                                 "__wue_b__" = "__wue~b~__",
                                 "__wue_t__" = "__wue~t~__")) %>% 
  rename(`**Variable**` = `**Characteristic**`) %>% 
  knitr::kable(caption = "Treatment comparison for seventeen variables between Well-Watered (WW) and Water Deficit (WD) in 15 potato genotypes. Where: Chlorophyll Concentration (spad), Plant height (hgt; cm), Relative water content (rwc; %), Leaf osmotic potential (lop; MPa), Leaf dry weight (ldw; g), Stem dry weight (sdw; g), Root dry weight (rdw; g), Tuber dry weight (tdw; g), Tuber number (ntub; N°), Total transpiration (trs; ml), Leaf area (lfa; cm^2^), Root length (rdl; cm), Total dry biomass (tdb; g), Harvest Index (hi), Specific Leaf Area (sla; cm^2^g^-1^), Relative Chlorophyll Content (rcc), Biomass water use efficiency (wue~b~; gl^-1^), Tuber Water Use Efficiency (wue~t~; gl^-1^). The values are represented by the mean ± standard deviation with the significance under t-test with their respective p-values.")
...
twue <- mc %>% 
  plot_gr(type = "bar", color = F,
          x = "gnt", x_lab =  "",
          y = "mean", y_lab = "Tuber water use efficiency (g.l^-1)",
          group = "treat", g_lab = "Treatment",
          g_brk = "water deficit (WD)  well watered (WW)",
          lgd = "none",
          y_lmt = c(0, 10),
...
```{r smrt, eval=FALSE, include=FALSE}
...
fb %>%
  ...
  group_by(treat, genotype) %>%
  summarise_all(funs(mean), na.rm = TRUE) %>%
  select(genotype, treat, everything()) %>%
  web_table(buttons = "none", title = "Supplementary table")
...
Gracias por los comentarios y las correciones del manuscrito, describí mejor la metodología de irrigación e incluí las dosis de fertilización.
...
Estoy incluyendo subtitulos a las discusiones como para resaltar la información principal y orgaizar mejor las discusiones.
    
Para la numeración del material suplementarios, como lo estoy escribiendo en R, las figuras y tablas son automaticamente numeradas de forma sucesiva, ya en la submission del articulo lo voy a corregir manualmente.

## Raymundo {-}

> Ajuste osmotico

De acuerdo a los sugeridó incluí "ajuste osmotico" como "loa", que es el división entre rwc/lop, esta variable esta siendo proyectanda de forma opuesta a las 2 variables individuales (rwc y lop) y la agrupación de los invididuos continua siendo la misma. Ya que en el cuadrante donde esta el cluster 5, estan los genotipos con mayor rwc y mayor potencial osmotico. Lo que puedo observar que ese indice esta más asociado a plantas bien regadas que a plantas en sequía.

La inclusion de esta variable no va modificar los resultados generales, más si tendria que incluir su información en los resultados y discusiones. Me comunica si de todas maneras es recomendable la inclusión de dicha variable en el manuscrito.

```{r , fig.cap= "loa = rwc/lop"}
mvd <- fb %>%
  select(-block, -gnt) %>%
  mutate(loa = rwc/lop) %>% 
  group_by(treat, genotype) %>%
...
ppi <- 300
png("files/pca_var_oa.png", width=8*ppi, height=8*ppi, res=ppi)
plot.PCA(pca,
         choix="var",
         title="",
         autoLab = "auto", 
...
ppi <- 300
png("files/pca_clu_oa.png", width=8*ppi, height=8*ppi, res=ppi)
...
pcv <- png::readPNG("files/pca_var_oa.png") %>%
  grid::rasterGrob(interpolate = TRUE)

pcc <- png::readPNG("files/pca_clu_oa.png") %>%
  grid::rasterGrob(interpolate = TRUE)
...
save_plot(plot = plot, "files/fig_mva_oa.png")
include_graphics("files/fig_mva_oa.png")
```

> FTSW vs TN

Este gráfico me pareción interesante. Pero no logre entender como hacerlo. En la figura \@ref(fig:srl) lo que estoy intentando mostrar a mi parecer es algo similar; la transpiración y la FTSW en el lapso del experimento

```{r}
include_graphics("files/tn.png")
```

Voy a compartirlos datos de la información de los cálculos de la irrigación para que me pueda ayudar con ese gráfico y poder adicionarlo al artíuclo como  supplementary information, seria bueno hacer uno para todos los genotipos juntos y mostrar la caida de FTSW vs TN
