...
tab <- xl %>% 
  ...
  tibble::add_row(Number = "caption"
                  , Genotypes = "Potato genotypes (*Solanum tuberosum* L.) used for water deficit experiment with two commercial varieties and 13 genotypes from advanced breeding population developed by the International Potato Center (CIP).")
...
tab <- fb %>%
  ...
  tibble::add_row(Variable = "caption"
                  , '**WD**, N = 75' = "Treatment comparison for seventeen variables between Well-Watered (WW) and Water Deficit (WD) in 15 potato genotypes. The values are represented by the mean ± standard deviation with the significance under t-test with their respective p-values.")

# tab %>% sheet_write(gs, data = .,  sheet = "stab2")
...
plot <- ggdraw(xlim = c(0, 0.5), ylim = c(0,0.8)) +
  draw_plot(plt1,  width = 0.495, height = 0.43, x = 0.004, y = 0.375) +
...
save_plot(plot = plot, "files/fig_srl.png", base_width = 6, base_height = 5)
...
spad_29 <- mc$table %>% 
  fplot(type = "line"
        ...
        , xlab =  ""
        , y = "spad_29"
        , ylab = "Chlorophyll concentration 29 dap (spad)"
        , group = "treat"
        , glab = "Treatment"
        , legend = "none"
        , limits = c(30, 70)
...
spad_83 <- mc$table %>% 
  fplot(type = "line", color = F
        ...
        , xlab =  ""
        , y = "spad_83"
        , ylab = "Chlorophyll concentration 83 dap (spad)"
        , group = "treat"
        , glab = "Treatment"
        , legend= "none"
        , limits = c(30, 70)
...
tab <- fb %>% means_lm_fb(trait = 5)

exp <- tab %>% 
  add_row(genotype = "caption"
          , treat = "Comparison of 15 potato genotypes under Well-Watered (WW) and Water Deficit (WD) condition. The values are represented by the mean with the significance difference under Student-Newman-Keuls test (p-value<0.05)."
          ) %>% 
  add_row(genotype = "notes"
          , treat = "Chlorophyll Concentration (spad), Plant height (hgt; cm), Relative water content (rwc; %), Leaf osmotic potential (lop; MPa), Leaf dry weight (ldw; g), Stem dry weight (sdw; g), Root dry weight (rdw; g), Tuber dry weight (tdw; g), Tuber number (ntub; N°), Total transpiration (trs; mL), Leaf area (lfa; cm2), Root length (rdl; cm), Total dry biomass (tdb; g), Harvest Index (hi), Specific Leaf Area (sla; cm2g-1), Relative Chlorophyll Content (rcc), Biomass water use efficiency (wueb; gL-1), Tuber Water Use Efficiency (wuet; gL-1)."
          )

# exp %>% write_sheet(gs, sheet = "stab1")
...
path <- paste0("h2_", file[1], "_", file[2], "_", file[3], ".xlsx")
h2sm %>% writexl::write_xlsx(path = path)
...
path <- paste0("blups_", file[1], "_", file[2], "_", file[3], ".xlsx")
blups %>% writexl::write_xlsx(path = path)
...
path <- paste0("blues_", file[1], "_", file[2], "_", file[3], ".xlsx")
blues %>% writexl::write_xlsx(path = path)
...
exp <- tab$h2 %>% 
  ...
  drop_na() %>% 
  tibble::add_row(variable = "caption"
                  , "mean ± std" = "Broad sense heritability in 15 potatos genotypes. Genetic variance (V.g), Error variance (V.e), Cullis (h2.c) and Piepho (h2.p) heritability") 

# exp %>% write_sheet(ss = gs, sheet = "stab2")  
