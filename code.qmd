...
author-title: "Flavio Lozano-Isla, Evelyn Roxana Farfan-Vignolo, Raymundo Gutierrez, Raul Blas, Khan Awais"
format:
  html:
    toc: true
    toc-location: left
...
"files/Fig1.tiff" %>% include_graphics()
```
...
"files/Fig2.tiff" %>% include_graphics()
```
...
"files/Fig3.tiff" %>% include_graphics()
```
...
"files/Fig4.tiff" %>% include_graphics()
```
...
ggsave2(plot = plot, "files/ESM_2.tiff", height = 25, width = 30, units = "cm")

"files/ESM_2.tiff" %>% include_graphics()
```
...
fig <- magick::image_read("files/potatos.png") %>% 
  grid::rasterGrob() %>%
  ggsave("files/ESM_3.tiff", plot =  ., width = 15, height = 15)

"files/ESM_3.tiff" %>% include_graphics()
```
